...
*Hinweis*: Nachdem Sie diese Richtlinie ausgewählt haben, müssen alle Knoten sein link:../maintain/rolling-reboot-procedure.html["Neu gestartet in einer rollenden Art und Weise"] Zum Aktivieren des NetApp Cryptographic Sicherheitsmoduls. Verwenden Sie *Maintenance* > *Rolling reboot*, um Neustarts zu initiieren und zu überwachen.

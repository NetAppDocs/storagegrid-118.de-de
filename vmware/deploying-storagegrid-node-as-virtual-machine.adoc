...
NOTE: Das temporäre Installationspasswort wird nur während der Knoteninstallation verwendet. Nachdem ein Knoten dem Grid hinzugefügt wurde, können Sie darauf zugreifen, indem Sie die link:../admin/change-node-console-password.html["Passwort für die Node-Konsole"] verwenden, die in der  `Passwords.txt` Datei im Wiederherstellungspaket aufgeführt ist.

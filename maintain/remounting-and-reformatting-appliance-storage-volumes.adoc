...
** In seltenen Fällen, z. B. wenn Sie vom technischen Support angewiesen werden oder wenn Sie wissen, dass für den Ersatz-Node weniger verfügbare Volumes für Objekt-Storage als der ursprüngliche Node verfügbar sind, müssen Sie dies tun link:restoring-object-data-to-storage-volume-for-appliance.html["Manuelles Wiederherstellen von Objektdaten"] Verwenden der `repair-data` Skript: Wenn einer dieser Fälle zutrifft, antworten Sie `n`.
+
[NOTE]
====

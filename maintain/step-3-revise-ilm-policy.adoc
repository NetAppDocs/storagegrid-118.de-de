...
.Bevor Sie beginnen
Sie haben ein gutes Verständnis davon, wie Sie link:../ilm/index.html["Managen von Objekten mit ILM"]. Sie sind mit der Erstellung von Storage-Pools und ILM-Regeln sowie der Simulation und Aktivierung einer ILM-Richtlinie vertraut.
...
. Wenn keine Richtlinien aufgeführt sind, wählen Sie *Weiter*, um zu zu gehen link:step-4-remove-ilm-references.html["Schritt 4: Entfernen Sie ILM-Referenzen"].
